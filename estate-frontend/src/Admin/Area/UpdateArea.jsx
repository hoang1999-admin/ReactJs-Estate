...
class UpdateArea extends Component {
    ...
    componentDidMount() {
        this.areaService.getAreaById(this.state.id).then((res) => {
            let areadata = res.data;
            this.setState({
              
                titleString: areadata.titleString,
                imageString: areadata.imageString,
            
                productidLong: areadata.productidLong,
    ...
    savearea = (e) => {
        ...
        if (area == null || this.state.id == '') {
            alert('Lưu thất bại');
        } else {
            this.areaService.updateArea(area, this.state.id).then(res => {
                                ...
                                <section class="content">
                                    <div class="card">
                                        <div class="card-header">
                                            <h3 class="card-title">
                                                <strong class="text-danger">DANH SÁCH CẬP NHẬT QUỐC GIA</strong>
                                            </h3>
